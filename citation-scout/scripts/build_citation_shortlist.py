...
def _quarto_extract_script() -> Path:
    return Path(__file__).resolve().parents[2] / "quarto-paper" / "scripts" / "extract_citation_gaps.py"
...
def claims_from_input(input_path: Path, *, max_claims: int) -> list[dict[str, Any]]:
    ...
    if not script.exists():
        raise FileNotFoundError(f"Could not find quarto-paper extractor at {script}")
