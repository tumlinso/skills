...
def retire_run_batch_in_transaction(conn: sqlite3.Connection, revision: int, *, project: Mapping[str, Any], request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    successor = conn.execute("SELECT id FROM workflow_runs WHERE id=?", (value["successor_run_id"],)).fetchone()
    ...
    run_tasks = {str(row[0]) for row in conn.execute(
        "SELECT task_id FROM workflow_lane_tasks lt JOIN workflow_lanes l ON l.id=lt.lane_id WHERE l.run_id=?", (source["id"],)
    )}
    ...
    active = conn.execute(f"SELECT task_id FROM workflow_lane_tasks WHERE task_id IN ({placeholders}) AND state='active'", task_ids).fetchall()
    ...
    for consumer in consumers:
        ...
        if row and row["status"] not in TERMINAL:
            nonterminal.append({"task_id": str(consumer["task_id"]), "prerequisite_task_id": str(consumer["prerequisite_task_id"])})
    if nonterminal:
    ...
    conn.execute(f"UPDATE workflow_lane_tasks SET state='skipped',completed_at=?,revision=? WHERE task_id IN ({placeholders}) AND state='queued'", (now, revision, *task_ids))
    ...
    receipt = {"status": "retired", "changed": True, "source_run_id": value["source_run_id"], "successor_run_id": value["successor_run_id"], "changed_task_ids": task_ids, "preserved_task_ids": [], "pre_revision": current_revision, "post_revision": revision, "reason": value["reason"]}
