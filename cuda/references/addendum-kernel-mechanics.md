...
- on V100, the unfused path would pay extra HBM round trips that CUDA Graphs
  cannot remove
...
Do not use CUDA Graphs as an excuse to keep a decomposition that still rereads
and rewrites full tensors through HBM. Graphs reduce launch overhead, not
memory-pass cost.
...
- on V100 specifically, repeated HBM passes usually deserve a fusion-first
  response before launch-side tricks
...
- CUDA Graphs do not make repeated HBM traffic cheap
...
Resume the main `cuda` workflow when:
