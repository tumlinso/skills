...
For Volta custom ops, those conditions should be checked early, not only after
prolonged library tuning. If the op exists to keep stable Tensor Core tile math
and fused control in one owner, WMMA can be the right implementation path.
...
For custom ops on Volta, bias earlier toward CUTLASS when the extension needs a
Tensor Core-capable owner instead of a thin library wrapper.
