...
On V100, keep the order of those levers straight: reduce HBM passes first, then
use CUDA Graph capture for whatever stable launch train remains. Graph capture
does not rescue a decomposition that keeps bouncing full tensors through HBM.
...
For custom ops on Volta, move to CUTLASS earlier when the point of the op is to
own fused tile math, blocked layout control, or repeated epilogue structure
rather than to keep wrapping many library launches.
...
For Volta custom ops, use that cuBLASLt run as the baseline, then move earlier
to CUTLASS or WMMA when the extension boundary exists to keep Tensor Core tile
math fused with stable custom logic.
...
On Volta, do not confuse "use Tensor Cores" with "stay library-backed." For
Tensor Core-eligible custom ops, owning the Tensor Core kernel can be the right
answer sooner than it would be on newer families.
...
On V100, assume repeated HBM reads and writes are more expensive than launch
overhead often makes them look. CUDA Graphs can help the launch side, but they
do not pay down full-memory traffic.
...
On V100, if the choice is between a fused path and a cleaner split path that
keeps rereading and rewriting full tensors through HBM, bias toward fusion
unless register spills, occupancy collapse, or library ownership clearly defeat
it.
