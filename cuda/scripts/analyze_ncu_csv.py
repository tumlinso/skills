...
def recommend_route(kernel_class: str) -> tuple[str, str]:
    if kernel_class == "memory-bound":
        return "fusion", "The next decision is bytes moved, fusion depth, or memory-pass removal."
    if kernel_class == "compute-path mismatch":
        return "tensor", "The next decision is Tensor Core routing, dtype, blocking, or library-vs-owned-kernel choice."
    if kernel_class in {"register-limited", "shared-memory-limited", "compute-heavy"}:
        return "hot-kernel", "Structure is mostly set and the next question is kernel-local tuning."
    return "profile", "The kernel summary is mixed, so pair it with timeline or benchmark evidence first."
...
def build_summary(path: Path) -> dict:
    ...
    route, route_reason = recommend_route(hot_class)
...
def format_summary(summary: dict) -> str:
    ...
    lines.append(f"- Recommended route: {summary['recommended_route']}. {summary['recommended_route_reason']}")
