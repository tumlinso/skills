...
def recommend_route(status: str, hints: list[str], kernel_rows: list[dict[str, str]]) -> tuple[str, str]:
    if status == "rerun":
        return "rerun", "Need a usable timeline before routing deeper."
    if "host-device-traffic" in hints:
        return "pipeline", "Memcpy or memset activity is large enough to route into pipeline repair first."
    if "allocator-churn" in hints:
        return "rerun", "Allocator churn is distorting the measured window."
    if "launch-overhead" in hints and kernel_rows:
        hottest = kernel_rows[0]
        avg_us = ns(hottest, "Avg (ns)") / 1_000.0
        instances = int(parse_float(hottest.get("Instances")))
        if avg_us <= 30.0 and instances >= 50:
            return "graphs", "Short stable kernels suggest graphs or grouped launches after fusion triage."
        return "fusion", "Launch trains are visible and the next question is fusion vs grouped execution."
    return "hot-kernel", "Timeline is usable and the next question is kernel-local cause."
...
def build_summary(stats_dir: Path) -> dict:
    ...
    route, route_reason = recommend_route(status, hints, kernel_rows)
    api_total = api.total_time_ns if api.total_time_ns > 0.0 else 1.0
...
def format_summary(summary: dict) -> str:
    ...
    lines.extend(
        [
            "",
            f"recommended_route: {summary['recommended_route']}",
            f"recommended_route_reason: {summary['recommended_route_reason']}",
